...
class RetrieveEPCForCNCodeConnectorStub @Inject()(
                                                 http: HttpClient,
                                                 val config: AppConfig) extends RetrieveEPCForCNCodeConnector with BaseConnector {

  private val responseHttpReads: HttpReads[Either[ErrorResponse, Seq[EPCInformation]]] = (method: String, url: String, response: HttpResponse) => {
    response.status match {
      case OK =>
        response.validateJson[Seq[EPCInformation]] match {
  ...
  def retrieveEPCForCNCode(cnCode: String)(implicit ec: ExecutionContext, hc: HeaderCarrier): Future[Either[ErrorResponse, Seq[EPCInformation]]] = {
    val url: String = s"${config.stubUrl()}/cn-code-epc/$cnCode"
